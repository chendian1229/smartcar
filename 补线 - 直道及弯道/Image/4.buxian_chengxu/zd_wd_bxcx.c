...
备    注： 无特殊要求时给形参start_i传入79即可
**********************************************************************************************/
void zhidao_wandao_bxcx(uint8 img_bin[][IMG_W_USED],uint8 start_i)
{
  int16 Ci;
  
  uint8 nomeaning_l;//用于水平从中间往两边提取跳变沿
  ...
  start_bj_j_left=0;
  start_bj_j_right=0;
  for( Ci=start_i ; Ci>=(start_i-4); Ci-- )//假如start_i==79 则是79 78 77 76 75 共计5行
  {
  ...
  liang_dian_lian_xian(img_bin,1,mid_point[7],mid_point[8]);
  liang_dian_lian_xian(img_bin,1,mid_point[8],mid_point[9]);
