...
SaiDao_type_m  pdsdlx(uint8 img_bin[][IMG_W_USED])
{
  ...
  f__edge_sp_czj(img_bin[Hang_3],93-10,&f_l_3,&left_3,&f_r_3,&right_3);//水平扫第三行
  f__edge_sp_czj(img_bin[Hang_2],93-15,&f_l_2,&left_2,&f_r_2,&right_2);//水平扫第二行
  f__edge_sp_czj(img_bin[Hang_end],93,&f_l_end,&left_end,&f_r_end,&right_end);//水平扫最后一行
  
  chuizhi_flag=f__edge_cz_czjdbxs(img_bin,30,20);
  ...
  liang_dian_lian_xian(img_bin,1,l_3_img,r_3_img);
  
  liang_dian_lian_xian(img_bin,125,czs1,czs2);
//  //环形判断线连线
...
/*-----------------------------------------------------*/  
  ...
  kuandu=r_end_img.j-l_end_img.j;                              //和现在这一帧最后一行宽度之差
    ...
    if((((f_l_3==0)&&(f_r_3==0))&&(chuizhi_flag==0))||(leixing_flag==1))               //else中为十字，第三行丢线
  {
    //判断为十字
  ...
  if(((f_l_3==1)||(f_r_3==1))&&(leixing_flag==0))    //满足条件则为直道、弯道、障碍、起跑线
  {
    //判断为直道 
    if((zhidao)&&(ruhuanxing_flag==0)&&(chuhuanxingbc_flag==0))   
    {
      gpio_set(PTD15, 0);
      sdlx_return=ZhiDao;
    ...
    //判断为弯道 
    if((wandao)&&(ruhuanxing_flag==0)&&(chuhuanxingbc_flag==0))
    {
      gpio_set(PTD15, 0);
      sdlx_return=WanDao;  
    ...
    if(((ruhuanxing)&&(chuhuanxing_flag==0)&&(chuizhi_flag==1))||(ruhuanxing_flag==1))
    {
                              //定义为入环形以后可以开启出环形
      gpio_set(PTD15, 0);
      gpio_set(PTE26,0);
