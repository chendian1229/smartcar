...
void liang_dian_lian_xian(uint8 img[][IMG_W_USED],uint8 gray_v,zb_imgarr_t I_1,zb_imgarr_t I_2)
{
  ...
  if( pd_ld_sf_ch_img(I_1,I_2)==0 )//说明两个点不重合 继续进行连线操作
  {
    slope_t slope;
    zb_imgarr_t I_t;
    zb_math_t M_1,M_2;
    ...
    {   
      ...
      for( M_t.x=M_1.x ; (M_t.x<=M_2.x)&&(M_t.x<=188) ; M_t.x++ )//函数莫名会卡在这里 暂时这样解决下
      {
      ...
      for( M_t.y=M_1.y ; M_t.y<=M_2.y ; M_t.y++ )
      {
...
{
  if( pd_ld_sf_ch_img(I_1,I_2)==0 )//说明两个点不重合
  {
    if( I_1.i!=I_2.i )//只有当两个点i值不同时 函数才会向上画延伸线
    {
      zb_imgarr_t I_t;
      ...
      else            //说明斜率存在
      {
        ...
        x_height=(uint8)( (hight_math-M_1.y)/slope.value + M_1.x );//公式y-y0=k*(x-x0)的变形
