...
void Car_Control(float TargetSpeed)
{
  ...
  //速度控制
  flag_sp++;
  if( flag_sp == 1 )//1不分频 2二分频 3三分频 以此类推
  {
    flag_sp=0;
    
    //速度控制周期:10ms*1=10ms
    Speed_PID_Value=Speed_PID( speed_average , TargetSpeed );
    //pwm_common = (int16)( -Angle_PID( angle_clhd+92 , 0 ) + Kp_an*Speed_PID_Value );//直接叠加
    pwm_common = (int16)( -Angle_PID( angle_clhd+92 , Speed_PID_Value ) );//串级PID
  }
  
  //方向控制
  flag_dir++;
  if( flag_dir == 5 )//1不分频 2二分频 3三分频 以此类推
  {
    flag_dir=0;
