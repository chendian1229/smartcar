...
void shizi_bxcx(uint8 img_bin[][IMG_W_USED])
{
  ...
  //if( (xgd_flag_l==1)||(xgd_flag_r==1) ) flg_msycsz=0;
  ...
  f_sgd_s_j_l = xgd_l.j;     //计算上述j值
  f_sgd_s_j_r = xgd_r.j;     //同上
  ...
  if( MAX(sgd_l.i,sgd_r.i)>69) flg_msycsz=1;
  ...
  if( flg_msycsz==0 )
  {
    ...
    zhidao_wandao_bxcx(img_bin,79);
  }
  else//flg_msycsz==1 马上要出十字了
  {
    zhidao_wandao_bxcx(img_bin,59);
  }
