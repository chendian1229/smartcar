...
/***************************************用户自定义区 开始***************************************/
#define IMG_W_YS   188 //原始图像宽度
#define IMG_H_YS   120 //原始图像高度

#define IMG_W_USED 188 //实际使用的图像宽度
#define IMG_H_USED 80  //实际使用的图像高度

#define IMG_TARGET_POSITION 88//用于方向环PID计算偏差的目标值

#define STAND_BALANCE_ANLGE 3.5
...
typedef struct
{
  uint8 exist;//0 斜率不存在 1 斜率存在
  float value;//斜率存在时的值 exist==0时 此值无意义
}slope_t;
...
{
  uint8 wz;//位置
  int8  sx;//属性 1 上升沿 -1下降沿
}edge_t;

//定义跳变沿个数(和跳变沿结构体数组配套使用)
typedef struct
{
  uint8 all;//所有跳变沿个数
  uint8 s_n;//    上升沿个数
