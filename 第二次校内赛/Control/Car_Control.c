...
void Car_Control(int16 MeasuredDir,int16 TargetDir,float TargetSpeed)
{
  ...
  //直立pwm
  if( bmkgflg_ctrspeed==1 )
  {
    pwm_common = (int16)( -Angle_PID( angle_clhd+cali_angle , STAND_BALANCE_ANLGE ) );//D1车模 和 E车模 此参数不一样
  }
  else
  {
    pwm_common = (int16)( -Angle_PID( angle_clhd+cali_angle , -4.0 ) );//D1车模 和 E车模 此参数不一样
  }
  ...
  if( flag_sp==1 )
  {
    ...
    speed_average=speed_average*1+speed_average_last*0;
    
    static uint8 gksydsj=0;//关控速一定时间
    static uint8 gksydsj_counter;//计时
    if( (fabs(speed_average-speed_average_last)>50)&&(gksydsj==0) )
    {gksydsj=1;gksydsj_counter=0;}
    
    speed_average_last=speed_average;
    
    extern uint8 en_SpdPID_output;
    extern float bias_integration_sp;//偏差积分
    if(gksydsj==1)
    {
      gksydsj_counter++;
      
      if( en_SpdPID_output==1 ) en_SpdPID_output=0;
      
      if(gksydsj_counter==5)//5*100=500ms
      {
        if(en_SpdPID_output==0)
        {
          en_SpdPID_output=1;bias_integration_sp=0;
        }
        if(gksydsj==1) gksydsj=0;
      }
    }
  ...
  {
    Ctr_Motor_L(pwm_common+pwm_dir);
    Ctr_Motor_R(pwm_common-pwm_dir);
  }
}
/**********************************************************************************************
