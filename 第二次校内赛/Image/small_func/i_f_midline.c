...
{
  uint8 midline_arr[5];
  midline_arr[0]=f__midline_frm_img(img_bxh,start_i-6,1);//最高
  midline_arr[1]=f__midline_frm_img(img_bxh,start_i-3,1);//次高
  midline_arr[2]=f__midline_frm_img(img_bxh,start_i  ,1);//中
  midline_arr[3]=f__midline_frm_img(img_bxh,start_i+3,1);//次低
  midline_arr[4]=f__midline_frm_img(img_bxh,start_i+6,1);//最低
  
//	uint8 temp;//用于交换变量
//	uint8 j,i; //j控制比较趟数 i控制每一趟比较次数
//	for (j = 0; j<5 - 1; j++)
//	{
//		for (i = 0; i<5 - 1 - j; i++)
//		{
//			if (midline_arr[i]>midline_arr[i + 1])
//			{
//				temp = midline_arr[i];
//				midline_arr[i] = midline_arr[i + 1];
//				midline_arr[i + 1] = temp;
//			}
//		}
//	}
  
  return (midline_arr[1]+midline_arr[2]+midline_arr[3])/3;
}