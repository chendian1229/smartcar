...
#define Hang_3    22     //        直道弯道判断
#define Hang_2    40     //        打方向
...
#define zhidao          (K_l_12.value>0)&&(K_r_12.value<0)//&&((f_l_3==1)&&(f_r_3==1))
...
SaiDao_type_m  pdsdlx(uint8 img_bin[][IMG_W_USED])
{
  ...
  slope_t K_l_13,K_r_13;                        //定义1、3行左右斜率
  ...
  static uint8 huanxing_baochi_flag=0;          //定义环形保持标志位
  static uint8 shizi_baochi_flag=0;             //定义十字保持标志位
         uint8 shizi_huanxing_flag=0;           //定义十字环形标志位，非静态变量

  f__edge_sp_czj_plus(img_bin[Hang_1],93,&f_l_1,&left_1,&f_r_1,&right_1);//水平扫第一行
  f__edge_sp_czj_plus(img_bin[Hang_2],93-40,&f_l_2,&left_2,&f_r_2,&right_2);//水平扫第二行
  f__edge_sp_czj(img_bin[Hang_3],93-40,&f_l_3,&left_3,&f_r_3,&right_3);//水平扫第三行
  ...
  l_3_img.i=Hang_3;     //  左2图像坐标赋值
  l_3_img.j=left_3;
  r_3_img.i=Hang_3;     //  右2图像坐标赋值
  r_3_img.j=right_3;
  ...
  l_3_math=covert_zb(l_3_img);     //第一行，第三行左右跳变沿坐标转换用于计算斜率
...
//  liang_dian_lian_xian(img_bin,125,l_2_img,r_2_img);
//  liang_dian_lian_xian(img_bin,125,l_3_img,r_3_img);  //直道弯道判断线
///*---------------------------------------------------------  */
//  
///*-------------------串口屏显示------------------------*/
...
//  UART_End();
//    Site_t site_lcd_d;
//    site_lcd_d.y=0+0;//angle_clhd
//    site_lcd_d.x=0;
//    LCD_num(site_lcd_d,f_l_end,BLACK,WHITE);
//    site_lcd_d.y=0+15;//直立pwm
//    site_lcd_d.x=0;
//    LCD_num(site_lcd_d,f_r_end,BLACK,WHITE);
...
/*---------------------------判断赛道类型-----------------------------------*/  
    ...
    //判断两边丢边，则为环形或者十字
    if(((f_l_2==0)&&(f_r_2==0))||(leixing_flag==1))
    {
            leixing_flag=1;  												//总类型置1可保持环形或者十字
            shizi_huanxing_flag=huanxing_shizi(img_bin,79,30);//十字环形判断标志位置位
            //进入环形条件且能保持
            if(((shizi_huanxing_flag==1)&&(shizi_baochi_flag==0)&&(leixing_flag==1))||(huanxing_baochi_flag==1))
            {                            
                gpio_set(PTD15, 0);
                gpio_set(PTE26,0);
                huanxing_baochi_flag=1;
                sdlx_return=RuHuanXing; 
                    
                //判断入环形是否结束
                //if((kuandu_last>125)&&(kuandu<125))
                ...
                if((dibu==1)&&(dibu_last==0))
                {
                        ...
                        gpio_set(PTE26,1);
                        leixing_flag=0;
                        huanxing_baochi_flag=0;
                        sdlx_return=ChuHuanXing;			//满足入环形结束程序后，切换出环形程序
                }
                    
            }
             //进入十字条件且能保持
            if(((shizi_huanxing_flag==0)&&(huanxing_baochi_flag==0)&&(leixing_flag==1))||(shizi_baochi_flag==1))
            {
                gpio_set(PTE26,0);
                shizi_baochi_flag=1;
                sdlx_return=ShiZi;
