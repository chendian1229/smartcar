...
备    注： “从右边界求左边界”
**********************************************************************************************/
uint8 f_Sedge_sp_cybjqzbj(uint8 line[],uint8 s_j)
{
  ...
  for( int16 Cj=s_j ; Cj>=0 ; Cj-- )//从右往左 故Cj--
  {
    //限幅 防止不在0~187范围内
    tmp_l = Cj;
    tmp_r = Cj+1;
    ...
    if( tmp_l<0 )   tmp_l=0;
    if( tmp_l>187 ) tmp_l=187;
    if( tmp_r<0 )   tmp_r=0;
    if( tmp_r>187 ) tmp_r=187;
...
备    注： “从左边界求右边界”
**********************************************************************************************/
uint8 f_Xedge_sp_czbjqybj(uint8 line[],uint8 s_j)
{
  uint8 yc_value_j;//扫描到的右侧j值 函数返回值
  
  uint8 flag=0;//是否找到右侧边界标志位
  
  int16 tmp_l;//被比较的左侧像素下标
  int16 tmp_r;//被比较的右侧像素下标
  
  for( int16 Cj=s_j ; Cj<=187 ; Cj++ )//从左往右 故Cj++
  {
    //限幅 防止不在0~187范围内
    tmp_l = Cj-1;
    tmp_r = Cj;
...
n_edge_t f__edge_sp_all(uint8 line[],edge_t edge[])
{
  ...
  #define SSDFW 35//少扫的范围 根据实际情况合理的设置这个值 可以减少误判 当然也可以直接设置为0
  for( uint8 Cj=0+SSDFW ; Cj<(IMG_W_USED-SSDFW-1) ; Cj++ )
  {
    if( line[Cj]!=line[Cj+1] )
    {
      ...
      {
        edge[n_edge.all-1].sx = 1;
        edge[n_edge.all-1].wz = Cj;
