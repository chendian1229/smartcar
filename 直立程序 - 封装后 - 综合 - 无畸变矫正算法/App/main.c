 ...
 void main()
{
  ...
  while(1)
  {
    while( uart_querychar(UART4,&sp) == 0 )
    {
      //测大循环时间开始
      pit_time_start(PIT2);
      ...
      printf("%d\t%d\t%d\t",left_2,right_2,right_2-left_2);
      #define tiaojian_1 (right_1-left_1)<=(right_2-left_2)
      #define tiaojian_2 (right_1-left_1)>(right_2-left_2)//正常赛道 上行宽度<下行宽度
      if( (tiaojian_2)&&(f_l_2==1)&&(f_r_2==1) )
      {
          position=(right_2+left_2)/2;          
        
      }
      
      if( (tiaojian_1)&&(f_l_2==1)&&(f_r_2==0) )
      {
        position=left_2+20;
      }
      
      if((tiaojian_1)&&(f_l_2==0)&&(f_r_2==1))
      {
        position=right_2-20;
      }
      
      if((tiaojian_1)&&(f_l_2==0)&&(f_r_2==0))
      {
