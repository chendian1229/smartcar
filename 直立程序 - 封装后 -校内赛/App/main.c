...
#define Hang_6    10     //6           往上扫的极限
#define Hang_51   33     //5的辅助行
...
uint8 f_r_51;
uint8 right_51;
uint16 color_51=BLUE;

uint8 middle_line;//赛道中线位置
uint16 color_ml=BLUE;

uint8 flag_jbp=0;//单边记步跑标志位：  0（默认）
...
extern    float speed_average;
    printf("%s\t",ftoa(speed_left,3));
    printf("%s\t",ftoa(speed_right,3));
    printf("%s\n",ftoa(speed_average,3));

//    
    //屏幕显示相关数据
    site.x=0;
    site.y=0;
    extern float pwm_speed;//Car_Control.c
    LCD_str(site,ftoa(pwm_speed,2), BLACK,WHITE);
    site.x=80;
    ...
    site.y=40;
    extern float speed_average;
    LCD_str(site,ftoa(speed_average,2), BLACK,WHITE);
    ...
    if( flag_jbp==0 )
    {
      ...
      if( (K_l>0)&&(K_r<0) )
      {
        ...
        if( (right_3-left_3)<15 )//说明遇到了起跑线/结束线
        {
          EncoderCounter_L=0;
          EncoderCounter_R=0;
          flag_jbp=1;
