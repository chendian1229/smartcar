...
float speed_right;
float speed_average;
float speed_average_last;
float Speed_PID_Value;
float Speed_PID_Value_for_Stand;
...
void Car_Control(float TargetSpeed)
{
  ...
  if( flag_sp == 10 )//1不分频 2二分频 3三分频 以此类推
  {
    ...
    //speed_average= speed_average * 0.3 + speed_average_last * 0.7;//对编码器的速度进行一阶低通滤波
   // speed_average_last = speed_average;
    
    
    pwm_speed = (int16)Speed_PID(speed_average , TargetSpeed);
   
    
  }
  
   pwm_common = (int16)( -Angle_PID( angle_clhd+93 ,0) );
  ...
  if( flag_dir == 2 )//1不分频 2二分频 3三分频 以此类推
  {
    ...
    //方向控制周期:5*10ms=50ms
    extern uint8 middle_line;//main.c
    pwm_dir = Dir_PID( middle_line , 95 );
  }
...
//  Ctr_Motor_R(100);
  ...
  Ctr_Motor_L(pwm_common+pwm_speed+pwm_dir);
  Ctr_Motor_R(pwm_common+pwm_speed-pwm_dir);
}